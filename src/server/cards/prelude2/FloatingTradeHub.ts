    ...
    super({
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          ...
          b.action('Add 2 floaters to ANY card.', (ab) => {
            ab.empty().startAction.resource(CardResource.FLOATER, 2);
          }).br;
          b.or().br;
          b.action('Remove any number of floaters here to gain that many of one standard resource.', (eb) => {
            eb.minus().text('X').resource(CardResource.FLOATER).startAction.text('X').wild(1);
          })          
        }),
      },
    });
